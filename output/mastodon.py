...
def post(item):
    ...
    for text_post in text_content:
        ...
        logger.debug(f"mastodon_client.status_post({text_post}, in_reply_to_id={reply_to_post}, media_ids={media_ids}, visibility={visibility}, language={language}), sensitive={item["sensitive"]}")
        a = mastodon_client.status_post(text_post, in_reply_to_id=reply_to_post, media_ids=media_ids, visibility=visibility, language=language, sensitive=item["sensitive"])
        logger.debug(a)
        reply_to_post = a["id"]
